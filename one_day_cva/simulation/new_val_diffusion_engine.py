...
from simulation.new_val_kernels import compile_cuda_bulk_diffuse, compile_cuda_compute_mtm, compile_cuda_diffuse_and_price, compile_cuda_oversimulate_defs, compile_cuda_generate_exp1, compile_cuda_nested_cva, compile_cuda_nested_im
...
class DiffusionEngine:
    def __init__(self, irs_batch_size, vanilla_batch_size, num_coarse_steps, dT, num_fine_per_coarse, dt, num_paths, num_inner_paths, 
        ...
        self.startAtPar = startAtPar
        self.nb_nested = nb_nested
        ...
        if not self.no_nested_cva:
            try:
                self.nested_cva = cuda.pinned_array(
                    (self.nb_nested+1, self.num_defs_per_path, self.num_paths), np.float32)
            except cuda.cudadrv.driver.CudaAPIError:
                print('couldn\'t allocate pinned array for nested_cva, using the numpy allocator instead (non-pinned array).')
                self.nested_cva = np.empty((self.nb_nested+1, self.num_defs_per_path, self.num_paths), np.float32)
            try:
                ...
                self.nested_cva_sq = np.empty((self.nb_nested+1, self.num_defs_per_path, self.num_paths), np.float32)

            try:
                self.nested_cva_save1 = cuda.pinned_array(
                    (self.nb_nested+1, self.num_defs_per_path, self.num_paths), np.float32)
            except cuda.cudadrv.driver.CudaAPIError:
                print('couldn\'t allocate pinned array for nested_cva_save1, using the numpy allocator instead (non-pinned array).')
                self.nested_cva_save1 = np.empty((self.nb_nested+1, self.num_defs_per_path, self.num_paths), np.float32)

            try:
                self.nested_cva_save2 = cuda.pinned_array(
                    (self.nb_nested+1, self.num_defs_per_path, self.num_paths), np.float32)
            except cuda.cudadrv.driver.CudaAPIError:
                print('couldn\'t allocate pinned array for nested_cva_save2, using the numpy allocator instead (non-pinned array).')
                self.nested_cva_save2 = np.empty((self.nb_nested+1, self.num_defs_per_path, self.num_paths), np.float32)
    ...
    def _allocate_device_arrays(self):
        ...
        if not self.no_nested_cva:
            ...
            self.d_nested_cva_save1 = cuda.device_array((self.num_defs_per_path, self.num_paths), np.float32)
            self.d_nested_cva_save2 = cuda.device_array((self.num_defs_per_path, self.num_paths), np.float32)
        if not self.no_nested_im:
