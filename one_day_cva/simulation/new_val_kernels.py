...
def compile_cuda_nested_cva(irs_batch_size, vanilla_batch_size, g_diff_params, g_R, g_L_T, num_fine_per_coarse, num_rates, num_spreads, num_defs_per_path, num_paths, num_inner_paths, max_coarse_per_reset, stream):
    ...
    sig = (nb.int32, nb.int32, nb.float32, nb.float32[:, :, :], nb.int8[:, :, :, :], nb.float32[:, :], nb.float32[:, :, :], nb.float32[:, :, :], nb.float32[:, :, :], nb.float32[:, :], nb.int32[:, :], nb.float32[:, :], nb.int32[:, :], nb.bool_[:, :], nb.float32[:, :, :], nb.from_dtype(xoroshiro128p_dtype)[:], nb.float32, nb.int32, nb.bool_, nb.float32[:, :], nb.float32[:, :], nb.float32[:, :], nb.float32[:, :])

    @cuda.jit(func_or_sig=sig, max_registers=64)
    def _cuda_nested_cva(coarse_start_idx, num_coarse_steps, t, X, def_indicators, dom_rate_integral, spread_integrals, mtm_by_cpty, cash_flows_by_cpty, irs_f32, irs_i32, vanillas_on_fx_f32, vanillas_on_fx_i32, vanillas_on_fx_b8, exp_1, rng_states, dt, window_length, indicator_in_cva, out1, out2, out3,out4):
        block = cuda.blockIdx.x
        block_size = cuda.blockDim.x
        tidx = cuda.threadIdx.x
        pos = tidx + block * block_size
        ...
        for i in range(num_cpty):
            for j in range(num_defs_per_path):
                ...
                cuda.syncthreads()
                if tidx == 0:
                    out3[j, block] += cva_payoff_by_cpty_sh[tidx]
                elif tidx == 1:
                    out4[j, block] += cva_payoff_by_cpty_sh[tidx]
                if 1<tidx:
                    k = (inner_stride) // 2
                    while k > 0:
                        if tidx < k:
                        ...
                        k //= 2
                    if tidx == 2:
                        out1[j, block] += cva_payoff_by_cpty_sh[2] / num_inner_paths
                        out2[j, block] += cva_payoff_by_cpty_sq_sh[2] / num_inner_paths
                    cuda.syncthreads()
        '''
        # Reduction routine for nested Monte-Carlo CVA
        # (assumes that inner simulations are entirely handled by one block)
        #print('new nb_num1 ', num_inner_paths)
        if tidx == 0:
            for j in range(num_defs_per_path):
                out1[j, block] = 0
                out2[j, block] = 0
        ...
        for i in range(num_cpty):
            for j in range(num_defs_per_path):
                ...
                cuda.syncthreads()
                k = (inner_stride-2) // 2
                #print('new nb_num ', num_inner_paths)
                #print('tidx ', tidx)
                if tidx == num_inner_paths-2:
                    out3[j, block] += cva_payoff_by_cpty_sh[tidx]
                    #print('value =', cva_payoff_by_cpty_sh[tidx])
                elif tidx == num_inner_paths-1:
                    out4[j, block] += cva_payoff_by_cpty_sh[tidx]
