...
class XVAEstimatorPortfolio(XVAEstimator):
    ...
    def _train(self, features_gen, labels_as_cuda_tensors, measure_exec_times, ignore=None, train_time = None):
        ...
        for t in timesteps:
            ...
            if train_time is None:
                ...
                if labels_std > 1e-8 and t in train_time:
                    if measure_exec_times:
                        exec_times['num_trainings'] += 1
                        exec_times['train'] -= time.time()
                    for i in range((max_iter+self._estimator.total_iter-1)//self._estimator.total_iter):
    ...
    def train(self, features_gen=None, labels_as_cuda_tensors=False, measure_exec_times=False, train_time = 10000):
        if features_gen is None:
            features_gen = self._build_features()
        exec_times = self._train(features_gen, labels_as_cuda_tensors, measure_exec_times, train_time = train_time)
        return exec_times
