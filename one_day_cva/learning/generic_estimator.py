...
class CNN_simple(nn.Module):
    def __init__(self, num_features, num_targets, hidden_size):
        ...
        self.max_po_c2 = nn.MaxPool1d(kernel_size=4, stride=3, padding=1)
...
class GenericEstimator:
    def __init__(self, input_dim, num_hidden_layers, num_hidden_units, num_samples, batch_size, num_epochs, \
        ...
        if not linear:
            ...
            #self.model = CNN_simple(input_dim, 1,num_hidden_units)
        self.linear = linear
        if device.type == 'cuda':
    ...
    def train(self, features, labels, shuffle=False, weights=None, max_iter=None, compute_heuristic=False, num_paths=None, valid_loss=True):
        ...
        #if self.linear: 
        #    self.t_features_mean = torch.zeros(features.shape[1], dtype=torch.float32, device=self.device)
        #    self.t_features_std = torch.ones(features.shape[1], dtype=torch.float32, device=self.device)
        #    self.t_labels_meanstd = torch.zeros(labels.shape[1], dtype=torch.float32, device=self.device)
        #    self.t_labels_std = torch.ones(labels.shape[1], dtype=torch.float32, device=self.device)
        #else:
        ...
        for e in bar:
            ...
            bar.set_postfix(loss=f'{total_loss :.5f}')    
            loss_l.append(np.log(total_loss))
            if self.best_sol:
